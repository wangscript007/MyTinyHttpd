...
int main()
{
    //默认是server_sock是失败
    int server_sock = -1;
    //设置监听的端口,端口号必须是一个至少16为的无符号整数
    u_short port = 0;
    int client_sock = -1;
    //sockaddr_in被包含在#include<netinet/in.h>
    //sockaddr_in是一个网际套接字结构
    struct sockaddr_in client_name;
    //socklen_t无符号32位整数
    socklen_t client_name_len = sizeof(client_name);

    //typedef unsigned long int pthread_t，是一个线程标识符
